...
int LightLevelCur = -1;   //переменная для хранения текущего (t) значения
//изначально она инициализированна как -1 чтобsы в случае не работы фоторезистора можно было сразу понять в чем проблема
int LightLevelLast = -1;  //переменная для хранения предыдущего (t-1) значения с фоторезистора 
...
void light_Level() {
  ...
  if (abs(LightLevelCur - LightLevelLast) > INTERVAL) {   //Проверка изменения полученного значения на указанный интервал
    ...
    Serial.print("Current light level: ");                 //Выводим надпись в консоль, а затем строкой ниже выводим значение сопротивления фототранзистора
    Serial.println(LightLevelCur);                        //Вывод полученного значения в консоль

    lcd.setCursor(7,1);                                   //Устанавливаем курсор на дисплее. Вторая строка, 8 позиция в этой строке
    lcd.print(LightLevelCur);                             //Выводим текущий уровень освещенности на дисплей
    lcd.setCursor(12,1);                                  //Устанавливаем курсор на дисплее. Вторая строка, 12 позиция в этой строке
    lcd.print("(Om)");                                    //Выводим единицы измерения фоторезистора (Ом)
  }
...
void setup(void)//стандартная функция для установки портов на запись/чтение
{
  ...
  lcd.print("Light:");  //Печатаем в этой позиции "Light : "
